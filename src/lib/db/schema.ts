import { pgTable, uuid, text, decimal, boolean, timestamp, integer } from 'drizzle-orm/pg-core'
...
export const orderItems = pgTable('order_items', {
  ...
  itemTypeId: uuid('item_type_id').references(() => itemTypes.id).notNull(),
  externalId: text('external_id'), // ID у поставщика
  quantity: integer('quantity').notNull(),
  purchasePrice: decimal('purchase_price', { precision: 10, scale: 2 }).notNull(), // Реальная цена в KGS
...
export const exchangeOperations = pgTable('exchange_operations', {
  id: uuid('id').defaultRandom().primaryKey(),
  clientId: uuid('client_id').references(() => clients.id).notNull(),
  amountForeign: decimal('amount_foreign', { precision: 12, scale: 2 }).notNull(), // Сумма в валюте клиента
  ...
  amountKgsReal: decimal('amount_kgs_real', { precision: 12, scale: 2 }).notNull(), // Сумма в KGS по реальному курсу
  amountKgsClient: decimal('amount_kgs_client', { precision: 12, scale: 2 }).notNull(), // Сумма в KGS по курсу клиента
  createdAt: timestamp('created_at').defaultNow(),
  notes: text('notes'),
})
